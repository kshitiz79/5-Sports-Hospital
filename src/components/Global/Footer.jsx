      ...
      <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-8 py-10 text-center md:text-left">
        ...
        <div className="flex flex-col items-center md:items-start">
          ...
          <div className="flex space-x-4 mt-4 text-green-500 text-xl">
            <a href="https://www.facebook.com/share/1BM9NaA5Th/" className="hover:text-green-600  transition-all"><FaFacebook /></a>
            <a href="https://www.linkedin.com/posts/5-sports-hospital_the-article-focuses-on-how-the-right-sports-activity-7308700337408811008-OGTp?utm_source=share&utm_medium=member_android&rcm=ACoAADpCLGQBwJLWvKUCfUEXskNd5ckGyVBLtgQ" className="hover:text-green-600  transition-all"><FaLinkedin /></a>
            <a href="https://www.instagram.com/5sportshospital?igsh=MXB2YnQ5d2wxcDNtYg==" className="hover:text-green-600  transition-all"><FaInstagram /></a>
            <a href="#" className="hover:text-green-600 transition-all"><FaYoutube /></a>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center md:items-start">
          ...
          <div className="h-1 w-16 bg-green-500 mt-2 mb-4 mx-auto md:mx-0"></div>
          <ul className="space-y-4 text-gray-400 max-w-md">
  <ContactItem 
    icon={<MdLocationOn />} 
...
const FooterLink = ({ href, label }) => (
  <li>
    <Link href={href} className="hover:text-black transition-all">
      {label}
    </Link>
  </li>
);
...
const ContactItem = ({ icon, text, type }) => {
  ...
  if (type === "email") {
    content = <a href={`mailto:${text}`} className="text-white hover:text-green-400">{text}</a>;
  } else if (type === "phone") {
    content = <a href={`tel:${text}`} className="text-white hover:text-green-400">{text}</a>;
  } else if (type === "location") {
    content = <a href={`https://www.google.com/maps/search/?q=${encodeURIComponent(text)}`} target="_blank" rel="noopener noreferrer" className="text-white hover:text-green-400">{text}</a>;
  } else {
